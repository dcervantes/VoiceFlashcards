...
function processWorkbook(workbook) {
  ...
  allPhrases = jsonSheet
    .slice(1)
    .map((row) => ({ level: row[0], phrase: row[1] }))
...
function goToStep2() {
  const numFlashcards = parseInt(
    document.getElementById("num-flashcards").value
  );
  const levelSliderValue = parseInt(
    document.getElementById("level-slider").value
  );
  const levels = ["A1", "A2", "B1", "B2", "C1", "C2"];
